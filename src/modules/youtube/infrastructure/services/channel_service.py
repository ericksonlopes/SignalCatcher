...
class ChannelService(IChannelService):
    ...
    def create_channel(self, channel_entity: ChannelEntity) -> ChannelEntity:
        ...
        if existing_channel:
            self.logger.warning(
                f"Attempt to duplicate channel: {channel_entity.url}",
                context={"url": channel_entity.url},
            )
        ...
        self.logger.debug(
            f"New channel created successfully: {new_channel.name}",
            context={"channel_id": new_channel.id, "channel_name": new_channel.name},
        )
        return new_channel
