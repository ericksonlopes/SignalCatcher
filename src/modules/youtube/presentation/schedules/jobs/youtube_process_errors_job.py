...
def process_errors_job():
    ...
    with ConnectorPostgres() as session:
        ...
        error_contents = (
            ...
            .filter(YoutubeContentModel.step == ContentStep.ERROR)
            .all()
        )
        ...
        for content in error_contents:
            ...
            try:
                ...
                if (
                    "members-only content like this video" in error_msg
                    or "members on level" in error_msg
                ):
                    ...
                    and "sign in if you've been granted access" in error_msg
                ):
                ...
                if (
                    ...
                    or "sign in to confirm you're not a bot" in error_msg
                ):
                    logging.critical(
                        "YouTube bot detection triggered! Stopping the system to prevent IP ban."
                    )
                    sys.exit(1)
